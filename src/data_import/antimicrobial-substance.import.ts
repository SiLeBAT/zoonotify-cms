...
interface AntimicrobialSubstanceEntity {
  name: string;
  publishedAt: Date;
  locale: string;
}
...
/**
 * Import antimicrobial substances from an Excel file.
 ...
 */
async function importAntimicrobialSubstances(strapi) {
  const filePath = path.join(__dirname, '../../../data/master-data/antimicrobialsubstance.xlsx');
  const logFilePath = path.join(__dirname, '../../../data/antimicrobialsubstance-import-log.json');
  ...
  const sheet = dataFromExcel.find((s) => s.name.toLowerCase() === 'antimicrobialsubstance');

  if (!sheet) {
    console.error('Sheet "antimicrobialsubstance" not found in the file');
    return;
  }
  if (sheet.data.length <= 1) {
    console.error('No data found in the "antimicrobialsubstance" sheet');
    return;
  }
  ...
  const collection = 'api::antimicrobial-substance.antimicrobial-substance';
    ...
    try {
      ...
      if (item.name_de && item.name_de.trim() !== '') {
        ...
        if (germanLocalization) {
          ...
          const germanRecord = await strapi.db.query('api::antimicrobial-substance.antimicrobial-substance').create({
            data: {
              name: item.name_de,
              publishedAt: new Date(),
              locale: 'de',
...
export { importAntimicrobialSubstances };