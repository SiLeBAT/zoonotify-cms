...
interface SuperCategorySampleOriginEntity {
  name: string;
  publishedAt: Date;
  locale: string;
}
...
async function importSuperCategorySampleOrigins(strapi) {
  ...
  /*
  console.log('Clearing existing super-category-sample-origin records...');
  const allRecords = await strapi.entityService.findMany('api::super-category-sample-origin.super-category-sample-origin', {
    locale: ['en', 'de']
  });
  for (const record of allRecords) {
    await strapi.entityService.delete('api::super-category-sample-origin.super-category-sample-origin', record.id);
  }
  ...
  */
  ...
  const collection = 'api::super-category-sample-origin.super-category-sample-origin';
    ...
    try {
      ...
      if (item.name_de && item.name_de.trim() !== '') {
        ...
        if (germanLocalization) {
          ...
          const germanRecord = await strapi.db.query('api::super-category-sample-origin.super-category-sample-origin').create({
            data: {
              name: item.name_de,
              publishedAt: new Date(),
              locale: 'de',
