...
async function importSamplingStages(strapi) {
  ...
  */
  ...
  const collection = 'api::sampling-stage.sampling-stage';
    ...
    try {
      ...
      if (item.name_de && item.name_de.trim() !== '') {
        ...
        if (germanLocalization) {
          ...
          const germanRecord = await strapi.db.query('api::sampling-stage.sampling-stage').create({
            data: {
              name: item.name_de,
              publishedAt: new Date(),
              locale: 'de',
